(()=>{
  ...
  style.textContent=`
    ...
    .qgm-modal{width:min(500px,100%);max-height:84dvh;overflow:auto;background:#fffaf5;border-radius:22px;padding:18px;box-shadow:0 20px 50px rgba(50,34,22,.25)}
    ...
    @media(max-width:390px){.fsx-wrap{padding-left:10px;padding-right:10px}.fsx-card{padding:14px}.fsx-head h2{font-size:19px}.qgm-modal{padding:16px}.qgm-grid{gap:7px}}
  `;
  ...
  view.querySelector('.fsx-settings').addEventListener('click',()=>{if(typeof window.switchView==='function')window.switchView('settingsView');});

  fetch('food-search-data.json?v=20260828b',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('load failed');return r.json();}).then(json=>{data=Array.isArray(json)?json:[];renderResults();}).catch(()=>{count.textContent='0件';results.innerHTML='<div class="fsx-empty">食品データを読み込めませんでした。</div>';});

  function switchSearchNav(){
    const settingsBtn=document.querySelector('.footnav button[data-view="settingsView"]');
    if(!settingsBtn) return;
    settingsBtn.dataset.view='foodSearchView';
    const label=settingsBtn.querySelector('.navLabel');
    if(label) label.textContent='食品検索'; else settingsBtn.textContent='食品検索';
    settingsBtn.setAttribute('aria-label','食品検索');
    const icon=settingsBtn.querySelector('.navIcon');
  ...
  function openGuideModal(){
    ...
    back.innerHTML=`<div class="qgm-modal" role="dialog" aria-modal="true" aria-label="四群の説明"><div class="qgm-head"><h2>四群点数法とは？</h2><button type="button" class="qgm-close" aria-label="閉じる">×</button></div><div class="qgm-text">食品を栄養の特徴ごとに4つの群に分け、食事全体のバランスを確認する考え方です。<div class="qgm-point">1点＝80kcal</div>「何点食べたか」だけでなく、「どの群から食べたか」を見ることで、食べ方の偏りに気づきやすくなります。</div><div class="qgm-grid"><div class="qgm-box qgm-g1"><b>第1群</b>牛乳・乳製品、卵など</div><div class="qgm-box qgm-g2"><b>第2群</b>魚介、肉、豆・豆製品など</div><div class="qgm-box qgm-g3"><b>第3群</b>野菜、きのこ・海藻、いも、果物など</div><div class="qgm-box qgm-g4"><b>第4群</b>主食、油脂、砂糖・調味料など</div></div></div>`;
