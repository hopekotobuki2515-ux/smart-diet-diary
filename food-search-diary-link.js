(()=>{
  ...
  const defs={
    1:['milk','egg'],
    2:['fish','meat','beans'],
    3:['veg','mush','potato','fruit'],
    4:['staple','seasoning','fat','other']
  };

  function roundHalf(n){return Math.round((Number(n)||0)*2)/2}
  function parseItem(card){
    const scores={1:0,2:0,3:0,4:0};
    card.querySelectorAll('.fsx-g').forEach(el=>{
      const m=(el.textContent||'').match(/第([1-4])群\s*([0-9.]+)点/);
      if(m) scores[Number(m[1])]=Number(m[2])||0;
    });
    return {
      name:(card.querySelector('.fsx-name')?.textContent||'食品').trim(),
      ingredients:(card.querySelector('.fsx-ing')?.textContent||'').replace(/^主な食材：/,'').trim(),
      scores
    };
  }
  ...
  function ensureMeal(m){
    if(!m.subCounts)m.subCounts={1:{},2:{},3:{},4:{}};
    [1,2,3,4].forEach(g=>{if(!m.subCounts[g])m.subCounts[g]={};defs[g].forEach(k=>{if(!Number.isFinite(Number(m.subCounts[g][k])))m.subCounts[g][k]=0})});
  }

  function addToDiary(item,mealId){
    if(typeof dayData!=='function') return false;
    const d=dayData(),m=d?.[mealId];
    if(!m) return false;
    ensureMeal(m);
    [1,2,3,4].forEach(g=>{
      const score=roundHalf(item.scores[g]);
      if(score<=0)return;
      const key=inferKey(g,item);
      m.subCounts[g][key]=roundHalf((Number(m.subCounts[g][key])||0)+score);
    });
    m.searchItems=Array.isArray(m.searchItems)?m.searchItems:[];
    m.searchItems.push({name:item.name,scores:item.scores,addedAt:Date.now()});
    try{
      if(typeof keyDate==='function'&&typeof current!=='undefined')localStorage.setItem('sdd-'+keyDate(current),JSON.stringify(d));
    }catch(e){}
  ...
  function openSheet(card){
    ...
    const back=document.createElement('div');back.className='fsd-backdrop';
    const scoreText=[1,2,3,4].filter(g=>item.scores[g]>0).map(g=>`<span>第${g}群 ${item.scores[g]}点</span>`).join('');
    back.innerHTML=`<div class="fsd-sheet" role="dialog" aria-modal="true"><div class="fsd-head"><div><h3>ダイアリーに追加</h3><div class="fsd-name">${item.name}</div></div><button class="fsd-close" aria-label="閉じる">×</button></div><div class="fsd-score">${scoreText}</div><div class="fsd-label">どの食事に追加しますか？</div><div class="fsd-meals">${Object.entries(mealNames).map(([id,label])=>`<button data-meal="${id}">${label}</button>`).join('')}</div></div>`;
  ...
  function enhance(){
    const root=document.querySelector('#foodSearchView .fsx-results');if(!root)return;
    root.querySelectorAll('.fsx-item').forEach(card=>{
      if(card.querySelector('.fsx-add'))return;
      const btn=document.createElement('button');btn.type='button';btn.className='fsx-add';btn.textContent='ダイアリーに追加';btn.onclick=e=>{e.preventDefault();openSheet(card)};card.appendChild(btn);
    });
  }

  const settings=document.querySelector('#foodSearchView .fsx-settings');if(settings)settings.remove();
  enhance();
  const results=document.querySelector('#foodSearchView .fsx-results');
  if(results){const mo=new MutationObserver(()=>enhance());mo.observe(results,{childList:true});}
})();