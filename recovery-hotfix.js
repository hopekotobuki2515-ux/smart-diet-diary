(()=>{
  ...
  style.textContent=`
    /* Restore the accepted diary card layout only */
    ...
    /* Visible food guide entry, scoped to the guide screen */
    ...
    @media(max-width:390px){
      ...
      .subItem .minus{width:48px!important;height:48px!important;line-height:44px!important}
  ...
  function fixNav(){
    const names={todayView:'ダイアリー',healthView:'体調',calendarView:'カレンダー',progressView:'進捗',settingsView:'設定'};
    document.querySelectorAll('.footnav button[data-view]').forEach(btn=>{
      const name=names[btn.dataset.view];
      if(!name) return;
  ...
  function exposeFoodGuide(){
    ...
    if(!box){
      ...
      box.className='repairFoodGuideBox';
      box.innerHTML='<div class="repairFoodGuideTitle">食材が何群か迷ったときはこちら</div>';
      const wheel=wrap.querySelector('.guideWheel');
      if(wheel) wrap.insertBefore(box,wheel); else wrap.appendChild(box);
    }
